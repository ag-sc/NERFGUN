...
public abstract class LabelRetriever {
	 ...
	 * @param k
	 */
	protected List<Instance> getDirectMatches(String searchTerm, String searchField, String returnField, int k,
		...
		try {
			...
			for (int i = 0; i < hits.length; ++i) {
				...
				if (d.get("freq") != null) {
					...
					// double similarity = stringSimilarity(searchField,
					// pAsString);
		...
		List<Instance> sortedResultsAsList = new ArrayList<>(result);
		sortedResultsAsList.sort(comparator);

		if (sortedResultsAsList.size() > k) {
			sortedResultsAsList = sortedResultsAsList.subList(0, k);
		}

		// result.clear();
		// result.addAll(sorted);

		return sortedResultsAsList;
	}
	 ...
	 * @param k
	 */
	protected List<Instance> getPartialMatches(String searchTerm, String searchField, String returnField, int k,
			Directory indexDirectory, StandardAnalyzer analyzer) {
		Set<Instance> result = new LinkedHashSet<>();
	...
	protected Set<String> sortBySimilarity(Set<String> input, String label, int topK) {
		...
		for (String s : input) {
			...
			// ontology namespaces are preferred 10.5 times more than property
			// ones, for ranking.
