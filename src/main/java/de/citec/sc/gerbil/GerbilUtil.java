...
public class GerbilUtil {
    ...
    public static GsonDocument bire2gson(de.citec.sc.corpus.Document document, boolean useGold) {
        GsonDocument gson = new GsonDocument();
        gson.text = document.getDocumentContent();
        gson.name = document.getDocumentName();
        if (useGold) {
            gson.annotations = document.getGoldStandard().stream().map(span -> GerbilUtil.bire2gson(span))
    ...
    public static de.citec.sc.corpus.Document gson2bire(GsonDocument gsonDocument, boolean useGold) {
    ...
    public static String bire2json(de.citec.sc.corpus.Document document, boolean useGold) {
        Gson gson = new Gson();
        if (useGold) {
            return gson.toJson(bire2gson(document, useGold));
        }
        return gson.toJson(bire2gson(document, useGold));
    }

    public static de.citec.sc.corpus.Document json2bire(String jsonDocument, boolean useGold) {
        Gson gson = new Gson();
        return gson2bire(gson.fromJson(jsonDocument, GsonDocument.class), useGold);
    }
}
